...
call_sumar_limpiando_impares: # a0:accum, a1:a, a2:b
   li t3, 0x55555555 #guardo el inmediato en t3
   #no puedo usar andi porque el inmediato es de 12 bits maximo.
   and a1, t3, a1
   add a0, a1, a2
	ret	
# Devuelve accum + a * b
...
call_multiplicar: # a0:accum, a1:a, a2:b
   beqz a1, cero #branch if equal zero
   blez a2, cero
   
sum:
   beqz a2, return #si b es cero salto a return
   add a0, a1, a0 #le sumo a accum "a", "b" veces con un loop.
   addi a2, a2, -1 #le resto uno a b, para decrementarlo
   j sum #salto a sum
...
cero:
    mv a0, x0 #le paso un cero a accum en este caso.
    ret
...
call_posicion_impar: # a0 a, a1 index, a2 length
   andi a0, a1, 0x1 #hago un and entre index y 1, si el primer digito se mantiene en uno era par, si queda en cero es impar
   beqz a0, ceroo #no entiendo para que esta lenght
   li a0, 0x1
	ret
ceroo:
    mv a0, x0 #devuelvo cero en a0 (registro para valores de retorno)
    ret
...
call_segunda_mitad: # a0 a, a1 index, a2 length
   li t3, 0x1 #cargo un 1 en t3 
   sra a2, a2, t3 #hago un shift right aritmetical (para mantener los bits significativos) 1 posicion, lo que es equivalente a dividir por dos.
	bge a1, a2, uno # si a1 (index) <= a2(length/2) entonces salto a uno
   mv a0, x0 #si no devuelvo cero en a0
   ret 
uno:
    li a0, 0x1
    ret
